...
function EditarDiretor() {
  ...
  const [diretor, setDiretor] = React.useState([]);

  const { id } = useParams();

  React.useEffect(() => {
    api.get(`/diretor/${id}`).then(({ data }) => {
      setDiretor(data);
    });
  }, []);
  ...
  const addPost = (data) =>
    api
      .patch(`http://localhost:3000/diretor/${id}`, data)
      .then(() => {
            ...
            <form onSubmit={handleSubmit(addPost)}>
              ...
              <div className="fields">
                ...
                <input
                  type="number"
                  name="anoNascimento"
                  placeholder={diretor.anoNascimento}
